...
class Predictor:
    ...
    def __init__(self, cfg, gpu_id=None):
        ...
        if cfg.DETECTION.ENABLE:
            print(f'come to predict detectron')
            self.object_detector = Detectron2Predictor(cfg, gpu_id=gpu_id)
    ...
    def __call__(self, task):
        ...
        print("inputs " + "="*20)
        print(len(inputs))
        print(f'first input shape is {inputs[0].shape}')
        print(f'second input shape is {inputs[1].shape}')
        if bboxes is not None:
        ...
        if self.cfg.DETECTION.ENABLE and not bboxes.shape[0]:
            ...
            print("preds is " + "="*20)
            print(preds.shape, f'max val from preds is {torch.max(preds)}')
            print(f'preds>0.0001 is {torch.sum(preds>0.0001)}') #253
        ...
        print(f"Add action_preds to task " + "="*20)
        print(task.action_preds.shape)
...
class ActionPredictor:
    ...
    def get(self):
        ...
        try:
            ...
            print("task returned from async_vis.get is"+"="*20)
