...
class AsyncVis:
    class _VisWorker(mp.Process):
        ...
        def run(self):
            ...
            while True:
                task = self.task_queue.get()
                print(f"task.id get from running AsycVis is {task.id}")
                print(f"task get from running AsycVis is {task.action_preds.shape}")
                if isinstance(task, _StopToken):
                    break
                print(f'come to draw_predictions with task.id: {task.id}')
                frames = draw_predictions(task, self.video_vis)
                print(f'result from draw_predictions is {len(frames)}')
...
def draw_predictions(task, video_vis):
    ...
    if boxes is not None:
        ...
        print("Go to video_vis.draw_clip_range function!" + "="*20)
        print(f"frames is {len(frames)}")
        print(f'keyframe_idx is {keyframe_idx}')
        print(f'draw_range is {draw_range}')
        frames = video_vis.draw_clip_range(
