...
#     class2idx = {
#     "watch (a person)":  1,
#     "talk to (e.g., self, a person, a group)":  2,
#     "listen to (a person)":  3,
#     "touch (an object)":  4,
#     "carry/hold (an object)":  5,
#     "walk":  6,
#     "sit":  7,
#     "lie/sleep":  8,
#     "bend/bow (at the waist)": 9
# }
    max_key = max(class2idx.values())
    # print(f"values if class2idx is {class2idx.values()}")
    # print(f"max_key is {max_key}")
    class_names = [None] * (int(max_key) + 1)
    ...
    print(f'len(class_names) from get_class_names is {len(class_names)}')
    # print(f'class_parent is {class_parent}')
    # print(f'subset_ids is {subset_ids}')
